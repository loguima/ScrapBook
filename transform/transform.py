...
class Transform:
    ...
    def browse(self):
        ...
        for category_name in self.extracted_dir.listdir():
            ...
            for product_name in listdir(category_extracted_dir):
                product_file = self.extracted_dir.path(category_extracted_dir, product_name)
                products.append(self.transform_product(product_file, category_name, product_name))
    ...
    def transform_product(self, product_file, category_name, product_name):
        ...
        elements[PRODUCT_PAGE_URL] = 'http:' + product_name.replace(SEPARATOR, '/')  # File name give url
